    ...
    <li className="movie_item">
      ...
      <span>
        {ratings.map((item,index) => {
          return <StarIcon className= {[styles.star, item.active && styles.active].join(' ')} onMouseOver ={() => handleMouseOver(item)}  />
        })}
      ...
      {props.savedMovies && props.savedMovies.length > 0 ? (
        <ul >
          {props.savedMovies.map((movie) => (
            <MovieItem movie={movie} onMovieDelete = {props.onMovieDelete} key={movie.id}/>
          ))}
        </ul>
      ) : (
        'No saved movies'
